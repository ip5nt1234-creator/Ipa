...
const fs = require('fs');
const Resigner = require('node-app-resigner');
...
app.post('/api/sign', upload.fields([
  ...
  try {
    ...
    if (!ipaFile || !p12File || !provFile) {
      return res.status(400).json({ error: 'يرجى رفع ملف IPA والشهادة والبروفجن.' });
    }
    ...
    const resigner = new Resigner({
      ipa: ipaFile.path,
      output: outputIpaPath,
      p12: p12File.path,
      passphrase: password || '',
      provisioning: provFile.path,
      bundleId: bundleId || undefined,
      displayName: appName || undefined
    });

    await resigner.resign();
...
<dict>
    ...
    console.error(err);
    res.status(500).json({ error: 'فشل التوقيع: تأكد من صحة الشهادة وكلمة المرور.' });
  }
});
